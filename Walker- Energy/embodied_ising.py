...
class ising:
	...
	def SensorIndex(self, x, xmax):
		return int(np.floor((x + xmax) / (2 * xmax + 10 *
                                    np.finfo(float).eps) * 2**(self.Ssize/4)))

	# Update the state of the sensor
	def UpdateSensors(self):
		self.speed_ind_leg1_j0 = self.SensorIndex(self.env.joints[0].speed/self.env.vmaxhip , self.env.vmaxhip*2)  
		self.sensors[0:4] = 2 * bitfield(self.speed_ind_leg1_j0, int(self.Ssize/4)) - 1
		self.speed_ind_leg1_j1 = self.SensorIndex(self.env.joints[1].speed/self.env.vmaxknee , self.env.vmaxknee*2)
		self.sensors[4:8] = 2 * bitfield(self.speed_ind_leg1_j1, int(self.Ssize/4)) - 1
		self.speed_ind_leg2_j0 = self.SensorIndex(self.env.joints[2].speed/self.env.vmaxhip , self.env.vmaxhip*2)
		self.sensors[8:12] = 2 * bitfield(self.speed_ind_leg2_j0, int(self.Ssize/4)) - 1
		self.speed_ind_leg2_j1 = self.SensorIndex(self.env.joints[3].speed/self.env.vmaxknee , self.env.vmaxknee*2)
		self.sensors[12:] = 2 * bitfield(self.speed_ind_leg2_j1, int(self.Ssize/4)) - 1
