...
class MountainCarEnv(gym.Env):
	...
	def step(self, action):
		...
		velocity += (action - 1) * 0.001 + (np.sin(np.arctan(np.cos(position)-(3*np.sin(3*position))-6*np.sin(2*position)))) * (-0.0025)
	...
	def reset(self):
		...
		self.state = np.array([-1.4, 0])	
		return np.array(self.state)

	def _height(self, xs):
		return (np.sin(xs)+np.cos(3*xs)+3*np.cos(2*xs)) * .45 + 2.5
...
#			self.viewer.add_geom(flag)
		...
		self.cartrans.set_rotation((np.cos(pos)-(3*np.sin(3*pos))-6*np.sin(2*pos))/4.30799)
