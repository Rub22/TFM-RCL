...
class ising:
	...
	def __init__(self, netsize, Nsensors=1, Nmotors=1):  # Create ising model
		...
		self.env.min_position = -2*np.pi
		self.env.max_position = 2*np.pi
		self.env.goal_position = 1.5 * np.pi
		self.env.max_speed = 4
	...
	def Move(self):
		...
		if self.env.state[0] >= self.env.max_position: 
			self.env.state = (self.env.min_position,self.env.state[1])

		elif self.env.state[0] <= self.env.min_position:
