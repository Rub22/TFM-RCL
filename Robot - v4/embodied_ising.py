...
class ising:
	...
	def Move(self):
		...
		# if self.env.state[5] >= self.max_angle_j0:  # Bounce when end of world is reached
		# 	if self.env.state[6] > 0:
		# 		self.env.state[5] = self.max_angle_j0
		# 		self.env.state[6] = 0
		# 	else:
		# 		self.env.state [5] = self.max_angle_j0

		# if self.env.state[5]<= self.min_angle_j0:  # Bounce when end of world is reached
		# 	if self.env.state[6] < 0:
		# 		self.env.state[5] = self.min_angle_j0
		# 		self.env.state[6] = 0
		# 	else:
		# 		self.env.state [5] = self.min_angle_j0
		#elif self.env.state[0] <= self.env.min_position:  # Bounce when end of world is reached
			#if self.observation[1] < 0:
		#	self.env.state = (self.env.max_position,self.env.state[1])
			#else:
	...
	def UpdateSensors(self):
		# self.angle_ind_leg1_j0 = self.SensorIndex(self.env.joints[0].angle, 2)  
		# self.sensors[0:4] = 2 * bitfield(self.angle_ind_leg1_j0, int(self.Ssize/4)) - 1
		# self.angle_ind_leg1_j1 = self.SensorIndex(self.env.joints[1].angle, 2)
		# self.sensors[4:8] = 2 * bitfield(self.angle_ind_leg1_j1, int(self.Ssize/4)) - 1
		# self.angle_ind_leg2_j0 = self.SensorIndex(self.env.joints[2].angle, 2)
		# self.sensors[8:12] = 2 * bitfield(self.angle_ind_leg2_j0, int(self.Ssize/4)) - 1
		# self.angle_ind_leg2_j1 = self.SensorIndex(self.env.joints[3].angle, 2)
		# self.sensors[12:] = 2 * bitfield(self.angle_ind_leg2_j1, int(self.Ssize/4)) - 1
		self.speed_ind_leg1_j0 = self.SensorIndex(self.env.joints[0].speed/self.env.vmaxhip , self.env.vmaxhip*1.3)  # UNCOMENT FOR UPDATE WITH VELOCITY
		self.sensors[0:4] = 2 * bitfield(self.speed_ind_leg1_j0, int(self.Ssize/6)) - 1
		self.speed_ind_leg1_j1 = self.SensorIndex(self.env.joints[1].speed/self.env.vmaxknee , self.env.vmaxknee*1.3)
		self.sensors[4:8] = 2 * bitfield(self.speed_ind_leg1_j1, int(self.Ssize/6)) - 1
